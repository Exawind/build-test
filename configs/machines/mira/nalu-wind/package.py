...
class NaluWind(CMakePackage):
    ...
    depends_on('mpi')
    depends_on('yaml-cpp~shared@0.5.3:')
    depends_on('trilinos~shared+exodus+tpetra+muelu+belos+ifpack2+amesos2+zoltan+stk+boost~superlu-dist+superlu+hdf5+zlib+pnetcdf+shards~hypre@master,develop')
    depends_on('openfast+cxx~shared', when='+openfast')
    depends_on('tioga~shared', when='+tioga')
