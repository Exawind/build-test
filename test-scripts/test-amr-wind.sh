...
test_configuration() {
  ...
  if [ "${MACHINE_NAME}" == 'rhodes' ] && [ "${COMPILER_ID}" == 'clang@10.0.0' ]; then
    ...
    cmd "cppcheck --template=gcc --inline-suppr --suppress=unusedFunction --suppress=unmatchedSuppression --std=c++14 --language=c++ --enable=all --project=compile_commands.json -j 32 --cppcheck-build-dir=cppcheck-wd -i ${AMR_WIND_DIR}/submods/amrex/Src -i ${AMR_WIND_DIR}/submods/googletest --output-file=cppcheck.txt"
  ...
  if [ "${MACHINE_NAME}" == 'eagle' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DAMR_WIND_ENABLE_CUDA:BOOL=ON -DAMReX_CUDA_ARCH:STRING=7.0 -DGPUS_PER_NODE:STRING=2 ${CMAKE_CONFIGURE_ARGS}"
