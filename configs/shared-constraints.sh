#!/bin/bash

TRILINOS="trilinos~alloptpkgs~amesos+amesos2~anasazi~aztec+belos+boost~cgns~dtk~epetra~epetraext+exodus~fortran~fortrilinos+gtest+hdf5~hypre~ifpack+ifpack2+instantiate~instantiate_cmplx~intrepid~intrepid2~isorropia+kokkos~metis~minitensor~ml+muelu~mumps~nox~openmp~phalanx~piro+pnetcdf~python~rol~rythmos~sacado+shards~shared+stk~suite-sparse+superlu~superlu-dist~teko~tempus+teuchos+tpetra~x11~xsdkflags+zlib+zoltan+zoltan2"

TRILINOS_PERCEPT="trilinos~alloptpkgs~xsdkflags~metis~mumps~superlu-dist+superlu~hypre+hdf5~suite-sparse~python~shared+boost+tpetra+epetra+epetraext+exodus+pnetcdf+zlib+stk+teuchos+belos+zoltan+zoltan2~amesos+amesos2~anasazi~ifpack+ifpack2+muelu~fortran~ml+gtest+aztec+sacado~x11+instantiate~instantiate_cmplx~dtk~fortrilinos~openmp~rol~nox+shards+intrepid~intrepid2+cgns"
