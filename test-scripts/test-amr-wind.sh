...
test_configuration() {
  ...
  if [ "${MACHINE_NAME}" == 'eagle' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DMPIEXEC_EXECUTABLE:STRING=srun -DAMR_WIND_ENABLE_CUDA:BOOL=ON -DAMReX_CUDA_ARCH:STRING=7.0 -DGPUS_PER_NODE:STRING=2 -DUNSET_TMPDIR_VAR:BOOL=TRUE -DCTEST_DISABLE_OVERLAPPING_TESTS:BOOL=TRUE ${CMAKE_CONFIGURE_ARGS}"
  fi
