...
if [ "${MACHINE}" == 'peregrine' ] || \
  ...
  (set -x; cp -R custom-package-files/boxlib ${SPACK_ROOT}/var/spack/repos/builtin/packages/)
  (set -x; cp -R custom-package-files/py-pillow ${SPACK_ROOT}/var/spack/repos/builtin/packages/)
  (set -x; cp -R custom-package-files/amrex ${SPACK_ROOT}/var/spack/repos/builtin/packages/)
